        ...
        return (
            <div className={"listmode-"+this.config.viewmode+" listtype-"+this.props.contenttype}>
                <div className="content-list-tools">
                    ...
                    <Actions fromview="list" parameters={{'contenttype':this.props.contenttype}} from={{id: this.props.id}} content={null} selected={this.state.list.list.filter((item)=>this.state.selected.includes(item.id))} actionsConfig={this.config.actions} afterAction={(refresh:boolean,config:any)=>this.afterAction(refresh,config)} />
                    {!this.config.show_table_header&&
                    <span>
