...
> {
  ...
  constructor(props: any) {
    ...
    this.state = {
      ...
      actionNew: false,
      currentPage: 0,
      sortby: this.config['sort_default'],
      selected: [],
      filter: [],
    };
  }
  ...
  componentDidUpdate(prevProps, prevState, snapshot) {
    ...
    if (
      ...
      this.getSortbyStr(prevState.sortby) !=
        this.getSortbyStr(this.state.sortby)
    ) {
      this.fetchData();
    }
  }
      ...
      <table className='table'>
        <tbody>
          ...
          {this.state.currentPage > 0 &&
            this.renderEmpties(this.config.pagination - data.length)}
        </tbody>
      </table>
    );
  }
  ...
  renderRows(list) {
    ...
    let renderCells = (content: any) => {
      return (
        <>
          {this.config.can_select && (
            <td
              onClick={() => this.select(content.id)}
              className='td-check center'
            >
              <input
                type='checkbox'
                checked={this.state.selected.includes(content.id)}
                value='1'
              />
            </td>
          )}
          <RenderProperties
            ...
            as='td'
          />
          {this.config['row_actions'].length > 0 && (
            <td className='list-row-tool'>
              <ListRowActions
                visibleNumber={this.config['row_actions_visible']}
                afterAction={(refresh: boolean) => this.afterAction(refresh)}
                from={{ id: this.props.id }}
                content={content}
                config={this.config['row_actions']}
              />
            </td>
          )}
        </>
    ...
    for (let i = 0; i < list.length; i++) {
      let content = list[i];
      let rowClasses = this.props.onRenderRow
        ? this.props.onRenderRow(content)
        : '';
      let canDD =
        this.config['can_dd'] &&
        content.priority != 0 &&
        this.state.sortby[0][0] == 'priority' &&
        this.state.sortby[0][1] == 'desc';
      if (canDD) {
        rows.push(
          <DDCard
            id={content.id}
            ...
            dropCard={(targetIndex: number) => this.dropCard(targetIndex)}
            key={content.id}
            className={rowClasses}
            onClick={(e) => this.linkClick(e, content)}
          >
            {renderCells(content)}
          </DDCard>
        );
  ...
  renderBlocks(list) {
    ...
    for (let item of list) {
      ...
      let rowClasses = this.props.onRenderRow
        ? this.props.onRenderRow(item)
        : '';
      cells.push(
        <div
          className={'blockview-cell ' + rowClasses}
          onClick={(e) => this.linkClick(e, item)}
        >
          <RenderProperties
            content={item}
            contenttype={this.props.contenttype}
        ...
        {this.config.show_header && (
          <h3>
            {this.config.show_header_icon && (
              <FieldtypeIcon contenttype={this.props.contenttype} />
            )}
            {this.state.def.name}({this.state.list.count})
          </h3>
        )}
        {(() => {
          ...
          {totalPage > 1 && (
            <span className='dm-pagination'>
              ...
              <a
                ...
                onClick={(e) => {
                  e.preventDefault();
                  this.setState({ currentPage: totalPage - 1 });
                }}
              >
              ...
              <span className='pagination-info'>
                Page {this.state.currentPage + 1} of {totalPage} from total{' '}
    ...
    return (
      ...
      >
        <div className='content-list-tools'>
          ...
          <Actions
            fromview='list'
            parameters={{ contenttype: this.props.contenttype }}
            from={{ id: this.props.id }}
            content={null}
            selected={this.state.list.list.filter((item) =>
              this.state.selected.includes(item.id)
            )}
            actionsConfig={this.config.actions}
            afterAction={(refresh: boolean, config: any) =>
              this.afterAction(refresh, config)
            }
          />
          {!this.config.show_table_header && (
            <span>
