  ...
  renderLink(config: any) {
    ...
    let linkText = (
      <>
        ...
        {!this.props.iconOnly && config.name ? config.name : ''}
      </>
    );
    ...
    return (
      <div className='action-item'>
        {!newWindow && (
          <Link
            to={path}
            title={this.props.iconOnly ? config.name : config.title}
          >
            {linkText}
          </Link>
        )}
        {newWindow && (
          <a href={path} title={config.title} target='_blank'>
            {linkText}
          </a>
        )}
      </div>
